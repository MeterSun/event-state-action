...
class State {
    ...
    exec(tagName, param) {
        const actions = this.actionSet.getAction(tagName);
        this._setNewData(param);
        actions.map(action => action(this.data, (tagName, param) => this.exec(tagName, param)));
    }
    _setNewData(param) {
        if (typeof param === "function") param = param(this.data, (tagName, param) => this.exec(tagName, param));
